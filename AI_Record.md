...
1. LLM 调用失败 / 网络连接问题
   用户：你好后出现 `LLM 调用失败：Connection error`，问是不是网络问题。
   AI：判断这是 LLM API 调用阶段失败，可能是 API Key、base\_url、网络、代理或服务商连接异常，不是业务代码本身。
   用户：要求处理代理问题，并提到想改 `.bashrc`，用 `clash no` 启动代理、`clash off` 关闭代理，而且默认关闭。
   AI：解释可以通过 shell alias/function 管理代理环境变量，默认不设置代理，需要时手动开启。
   方案：后端保留 LLM 异常处理；本地环境层面按你的建议用命令开关代理，避免默认代理影响所有请求。
2. `session_id` 从哪里来
...
6. 前端文件上传和发送按钮问题
   ...
   用户：要求我详细列出“上传文件到回答问题”的整体流程。
   AI：解释流程：前端选择文件 -> 创建 attachment 状态 -> XHR 上传 -> 后端保存和解析 -> 返回文档 metadata -> 前端标记 ready -> 发送时携带 `document_ids` -> 后端注入文档信息 -> LLM 调用 `read_docs`。
...
9. 超过 20 轮强制压缩
   用户：先说“不用改 Step 2，只保留最近 KEEP\_RECENT\_MESSAGES 条 + 注入压缩说明更合理”。
...
15. LLM 报错的对话不应保存和展示
    ...
    AI：确认当前逻辑确实先 append 了 user display，且 `run_agent_loop` 也会先把 user 写入 raw\_messages。
    AI：改成事务式提交：本轮开始前备份 raw\_messages，构建 user/assistant display 但不写入；如果出现 error，回滚 raw\_messages，不保存 display，不保存 trace；成功后统一提交。
