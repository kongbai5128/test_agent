...
- `todos`：本会话独立的待办事项列表
- `documents`：本会话上传过的文档元数据
- 多窗口完全隔离，互不影响

### 文档上传与读取

- 前端支持选择 PDF、Word、TXT/Markdown 文件，上传过程显示文件卡片和进度
- 后端将文档保存到 `data/docs/{session_id}/{document_id}/`
- 聊天请求可携带 `document_ids`，后端会把本轮附件注入 system prompt
- LLM 需要读取正文时调用 `read_docs`，优先使用 `document_id`
...
| POST   | /api/sessions/{id}/documents   | 上传文档附件           |
| GET    | /api/sessions/{id}/documents   | 获取会话文档列表       |
| DELETE | /api/sessions/{id}/documents/{doc_id} | 删除文档附件    |
